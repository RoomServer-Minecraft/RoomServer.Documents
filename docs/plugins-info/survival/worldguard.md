...
「WorldGuard」とは、指定エリアでのプレイヤーの行動を制限することで、範囲内の建造物を保護するプラグインです。  
コマンド操作により、ブロックの破壊やアイテムの使用などに、制限をかける事が出来ます。

{% hint style="success" %}
**WorldGuardで出来ることは次の通りです**
...
<table>
	...
	<tbody>
		<tr><td>`build`</td><td>保護領域内でオーナーとメンバー以外が建築出来る</td><td>deny</td></tr>
