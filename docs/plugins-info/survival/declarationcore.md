# DeclarationCore（宣戦布告システム）

## DeclarationCoreとは？

サバイバルサーバーでの戦争や宣戦布告、クラン（勢力）などのシステムを管理するためのプラグインです。  
コマンドや専用の画面を通じて、他のプレイヤーやクランに対して正当な手順で戦争を仕掛ける事が出来ます。

{% hint style="success" %}
**DeclarationCoreで出来ることは次の通りです**
...
このプラグインによって、Discordを持っていないプレイヤーでもゲーム内だけで完結して戦争に参加出来ます。  
ルールが明確化されることで安心してサバイバルを楽しむ事が出来ます！
...
また、初期スポーン周辺の保護エリアや、運営に承認された公共施設などは、戦争中であっても破壊することは出来ません。
...
<table>
	...
	<tbody>
		<tr>
			...
			<td>宣戦布告メニューを表示します。相手や条件（PvP/破壊）を設定出来ます。</td>
		</tr>
		<tr>
...
* **リーダー権限**: クランに所属している場合、宣戦布告や合意の判断が出来るのはリーダー（または設定された戦争リーダー）のみです。
